...
async function fetchLatestModels () {
  ...
  const latestBedrockModel = bedrockMatch[0]
  ...
  try {
    ...
    const actionUpdated = await updateFile(
      ...
      [
        ...
        {
          search: /bedrock_models: 'anthropic\.claude-sonnet-\d+(?:-\d+)?-\d{8}-v1:0'/,
    ...
    const bedrockUpdates = [
      {
        search: /models = \['anthropic\.claude-(?:3-7-sonnet-\d{8}|sonnet-\d+(?:-\d+)?-\d{8})-v1:0'\]/,
    ...
    if (needsCountTokensEntry) {
      bedrockUpdates.push({
        search: /('anthropic\.claude-sonnet-\d+(?:-\d+)?-\d{8}-v1:0': anthropicCountTokens,)\n/,
