...
Given('the first bedrock call rejects the token budget with {string}', function (message) {
  mockState().bedrock.sendErrors = [Object.assign(new Error(message), { name: 'ValidationException' })]
})
...
When('the bedrock patch embedding property holds for {int} runs', async function (runs) {
  ...
  await fc.assert(fc.asyncProperty(patchBody, async (body) => {
    const out = await explainPatch({ patchBody: body, owner: 'o', repo: 'r' })
