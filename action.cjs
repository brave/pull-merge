module.exports = async ({ github, context, inputs, actionPath }) => {
  ...
  const options = Object.assign({
    ...
    anthropic_models: 'claude-opus-5-5',
    bedrock_models: 'global.anthropic.claude-opus-5-5-v1:0',
  ...
  options.max_tokens = Number(options.max_tokens) || 16384
  ...
  try {
    ...
    const extraDiff = await getExtraDiff({ inputs, patch, debug })
    if (extraDiff) {
      if (debug) { console.log(`Appending ${extraDiff.length} bytes of extra diff from extra_diff_repository`) }
